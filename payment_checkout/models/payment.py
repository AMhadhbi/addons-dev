...
class PaymentAcquirerCheckout(models.Model):
    ...
    def checkout_form_generate_values(self, tx_values):
        ...
        _logger.info("######################### %s",checkout_tx_values)
        ...
        _logger.info("######################### returndata %s",temp_checkout_tx_values['returndata'])
        _logger.info("######################### %s",checkout_tx_values)
    ...
    def checkout_s2s_form_process(self, data):
        _logger.info("######################### checkout_s2s_form_process")
...
class PaymentTransactionStripe(models.Model):
    ...
    def _create_checkout_charge(self, acquirer_ref=None, tokenid=None, email=None):
        ...
        charge_params = {
            'value':int(self.amount if self.currency_id.name in INT_CURRENCIES else self.amount*100),
    ...
    def checkout_s2s_do_transaction(self, **kwargs):
        ...
        _logger.info("########################checkout_s2s_do_transaction")
    ...
    def _checkout_form_get_tx_from_data(self, data):
        ...
        if not reference:
            ...
            _logger.error('Stripe: invalid reply received from checkout API, looks like '
...
class PaymentTokenCheckout(models.Model):
    ...
    def checkout_create(self, values):
        ...
        _logger.info("#########################values %s",values.get('acquirer_id'))
          
        _logger.info("#########################checkout_create")
        
        payment_acquirer = self.env['payment.acquirer'].browse(values.get('acquirer_id'))
        
        _logger.info("#########################payment_acquirer %s ",payment_acquirer)
         
         
        url_token = 'https://%s/tokens/payment' % payment_acquirer._get_checkout_api_url()
        
        _logger.info("#########################url_token %s",url_token)
        
        url_customer = 'https://%s/customers' % payment_acquirer._get_checkout_api_url()
        
        _logger.info("#########################url_customer %s",url_customer)
        if values.get('cc_number'):
            _logger.info("#########################values.get('cc_number') %s",values.get('cc_number'))
            payment_params = {
                'card[number]': values['cc_number'].replace(' ', ''),
                'card[expiryMonth]': str(values['cc_expiry'][:2]),
                'card[expiryYear]': str(values['cc_expiry'][-2:]),
                'card[cvc]': values['cvc'],
                'card[name]': values['cc_holder_name'],
            }
            _logger.info("########################payment_params %s",payment_params)
            
            r = requests.post(url_token,
                              auth=(payment_acquirer.checkout_secret_key, ''),
                              params=payment_params)
            
            _logger.info("########################r %s",r.json())
