# -*- coding: utf-8 -*-
import logging
import pprint
import werkzeug

from odoo import http
from odoo.http import request

_logger = logging.getLogger(__name__)


class CheckoutController(http.Controller):

    @http.route(['/payment/checkout/s2s/create_json'], type='json', auth='public')
    def checkout_s2s_create_json(self, **kwargs):
        _logger.info("#########################checkout_s2s_create_json ")
        acquirer_id = int(kwargs.get('acquirer_id'))
        acquirer = request.env['payment.acquirer'].browse(acquirer_id)
        return acquirer.s2s_process(kwargs).id

    @http.route(['/payment/checkout/s2s/create'], type='http', auth='public')
    def checkout_s2s_create(self, **post):
        
        _logger.info("#########################checkout_s2s_create ")
             
        acquirer_id = int(post.get('acquirer_id'))
        acquirer = request.env['payment.acquirer'].browse(acquirer_id)
        acquirer.s2s_process(post)
        return werkzeug.utils.redirect(post.get('return_url', '/'))

    @http.route(['/payment/checkout/s2s/create_json_3ds'], type='json', auth='public', csrf=False)
    def checkout_s2s_create_json_3ds(self, verify_validity=False, **kwargs):
        token = request.env['payment.acquirer'].browse(int(kwargs.get('acquirer_id'))).s2s_process(kwargs)
        
        _logger.info("#########################checkout_s2s_create_json_3ds ")
         
        if not token:
            res = {
                'result': False,
            }
            return res

        res = {
            'result': True,
            'id': token.id,
            'short_name': token.short_name,
            '3d_secure': False,
            'verified': False,
        }

        if verify_validity != False:
            token.validate()
            res['verified'] = token.verified

        return res

    @http.route(['/payment/checkout/create_charge'], type='json', auth='public')
    def checkout_create_charge(self, **post):
        """ Create a payment transaction

        Expects the result from the user input from checkout.js popup"""
        
        _logger.info("#########################checkout_create_charge ")
        TX = request.env['payment.transaction']
        tx = None
        
                
        if post.get('tx_ref'):
            tx = TX.sudo().search([('reference', '=', post['tx_ref'])])
        if not tx:
            tx_id = (post.get('tx_id') or request.session.get('sale_transaction_id') or
                     request.session.get('website_payment_tx_id'))
            tx = TX.sudo().browse(int(tx_id))
        if not tx:
            raise werkzeug.exceptions.NotFound()
        response = tx._create_checkout_charge(tokenid=post['tokenid'], email=post['email'])
        _logger.info('Checkout: entering form_feedback with post data %s', pprint.pformat(response))
        
        _logger.info("#########################response %s",response)
        if response:
            request.env['payment.transaction'].sudo().form_feedback(response, 'checkout')
        return post.pop('return_url', '/')
